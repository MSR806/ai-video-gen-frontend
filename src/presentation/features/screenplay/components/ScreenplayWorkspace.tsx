...
import { ensureSceneHasBlocks, makeBlock, reorder, validateScene } from './screenplay-editor.utils';
import {
...
export function ScreenplayWorkspace({
    ...
    (sceneId: string, content: ScreenplayScene['content']) => {
      setScreenplay((previous) => {
        ...
        const nextScenes = previous.scenes.map((scene) => {
          if (scene.id !== sceneId || isBlockListEqual(scene.content.blocks, content.blocks)) {
            return scene;
          }
  ...
  const flushPendingSceneUpdates = useCallback(async () => {
    ...
    const flushPromise = (async () => {
      ...
      try {
        ...
        const provisionalScenePatches = queued.filter(([sceneId]) => isProvisionalSceneId(sceneId));
  ...
  const characterCueSuggestions = useMemo(() => {
    ...
    return Array.from(
      new Set(
        screenplay.scenes
          .flatMap((scene) => scene.content.blocks)
  ...
  const handleAddScene = async () => {
    ...
    try {
      ...
      const created = await addUseCase.execute(projectId, {
        ...
        content: { blocks: [makeBlock('slugline'), makeBlock('action')] },
      });
        ...
        <div className={styles.sceneList}>
          {screenplay.scenes.map((scene, index) => {
            ...
            return (
              ...
              >
                <span className={styles.sceneListTitle}>{scene.name || `Scene ${index + 1}`}</span>
                {warnings > 0 ? (
            ...
            {activeSceneWarnings.length > 0 ? (
              <ul className={styles.warningList}>
                {activeSceneWarnings.map((warning) => (
                  <li key={`${warning.blockId}:${warning.message}`}>
...
function resolveSceneIdForMutation(
  ...
  const beforeSlugline = beforeFlushScene.content.blocks.find(
    (block) => block.type === 'slugline',
  )?.text;
  if (beforeSlugline) {
    const bySlugline = afterFlushScenes.find(
      (scene) =>
        scene.content.blocks.find((block) => block.type === 'slugline')?.text === beforeSlugline,
    );
    if (bySlugline) {
...
function sceneIdentitySignature(scene: ScreenplayScene): string | null {
  const slugline = scene.content.blocks.find((block) => block.type === 'slugline')?.text?.trim();
  if (!slugline) {
    return null;
  }

  const firstActionOrDialogue = scene.content.blocks
