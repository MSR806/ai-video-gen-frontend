import type { CameraBody, Lens, FocalLength } from '../domain/asset.entity';
...
export const DEFAULT_CAMERA_SETUP = {
  camera: CAMERA_BODIES[0], // IMAX
  lens: LENSES[0], // Zeiss Ultra Prime
  focalLength: FOCAL_LENGTHS[1], // 24mm
};
...
export function getCameraById(id: string): CameraBody | undefined {
  return CAMERA_BODIES.find((c) => c.id === id);
}
...
export function getFocalLengthByValue(value: number): FocalLength | undefined {
  return FOCAL_LENGTHS.find((f) => f.value === value);
}
